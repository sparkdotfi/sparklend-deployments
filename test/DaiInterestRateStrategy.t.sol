...
contract DaiInterestRateStrategyTest is DssTest {
    ...
    function test_calculateInterestRates_no_maker_debt_with_borrows() public {
        ...
        assertRates(
            ...
            0,
            BR + 50 * RBPS,
            "No Maker debt, user-only supply under performance bonus. supply = 0, borrow = dsr + 50bps"
        );
        assertRates(
            200_000 * WAD,
            (BR + 25 * RBPS) * 1 / 2 * 1 / 2,
            BR + 50 * RBPS,
            "No Maker debt, user-only supply over performance bonus. supply = 0, borrow = dsr + 50bps"
        );
    }
    ...
    function test_calculateInterestRates_maker_debt_with_borrows() public {
        ...
        assertRates(
            ...
            0,
            BR + 50 * RBPS,
            "Only Maker as LP, borrows under performance bonus. supply = 0, borrow = dsr + 50bps"
        );
        ...
        assertRates(
            ...
            (BR + 25 * RBPS) * 1 / 2,
            BR + 50 * RBPS,
            "Only Maker as LP, borrows over performance bonus. supply = 0, borrow = dsr + 50bps"
        );
    }
    ...
    function test_calculateInterestRates_over_debt_limit() public {
        ...
        assertRates(
            ...
            br,
            br,
            "Only Maker as LP, 2x over capacity, 100% utilization. supply = ~maxRate/2, borrow = ~maxRate/2"
        );
        dai.setLiquidity(1_000_000 * WAD); // User adds some liquidity, still over capacity
        assertRates(
            2_000_000 * WAD,
            255351974390643652177234692,
            br,
            "Maker+Users as LP, 2x over capacity, 66.7% utilization. supply = ~maxRate/2 * 66.7%, borrow = ~maxRate/2"
        );
    }
    ...
    function test_calculateInterestRates_debt_ceiling_zero() public {
        ...
        assertRates(
            100_000 * WAD, br, br, "Maker wants to go to zero debt - always maxRate. supply = maxRate, borrow = maxRate"
        );
    }
    ...
    function test_calculateInterestRates_vat_caged() public {
        ...
        uint256 br = 749999997628498784959732204; // Pretty much maxRate with some rounding errors
        assertRates(
            500_000 * WAD, br, br, "Should be near max rate when vat is caged. supply = maxRate, borrow = maxRate"
        );
    }
    ...
    function test_calculateInterestRates_fuzz(
        ...
        interestStrategy = new DaiInterestRateStrategy(
            address(vat), address(pot), ILK, baseRateConversion, borrowSpread, supplySpread, maxRate, performanceBonus
        );
        ...
        (uint256 supplyRate,, uint256 borrowRate) = interestStrategy.calculateInterestRates(
            DataTypes.CalculateInterestRatesParams(0, 0, 0, 0, totalVariableDebt, 0, 0, address(dai), address(0))
        );
        ...
        if (supplyRatio > 0) {
            ...
            assertGe(
                adjBorrowRate,
                supplyRate,
                "adjusted borrow rate should always be greater than or equal to the supply rate"
            );
        } else {
        ...
        assertGe(
            borrowRate,
            interestStrategy.getBaseRate() + borrowSpread,
            "borrow rate should be greater than or equal to base rate + spread"
        );
