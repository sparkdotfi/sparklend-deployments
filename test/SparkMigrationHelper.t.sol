...
contract MigrationHelperTest is Test {
    ...
    function testCacheATokens() public {
        for (uint256 i = 0; i < v3Reserves.length; i++) {
            DataTypesV2.ReserveData memory reserveData = migrationHelper.V2_POOL().getReserveData(v3Reserves[i]);
            ...
            uint256 allowanceToPoolV2 =
                IERC20(v3Reserves[i]).allowance(address(migrationHelper), address(migrationHelper.V2_POOL()));
            assertEq(allowanceToPoolV2, maxApproval);

            uint256 allowanceToPool =
                IERC20(v3Reserves[i]).allowance(address(migrationHelper), address(migrationHelper.V3_POOL()));
    ...
    function testMigrationNoBorrowWithPermit() public {
        ...
        // get positions
        (address[] memory suppliedPositions, uint256[] memory suppliedBalances,) = _getV2UserPosition(user);

        // calculate permits
        IMigrationHelper.PermitInput[] memory permits =
            _getPermits(user, privateKey, suppliedPositions, suppliedBalances);
    ...
    function testMigrationWithCreditDelegation() public {
        ...
        IMigrationHelper.RepaySimpleInput[] memory positionsToRepaySimple = _getSimplePositionsToRepay(positionsToRepay);
        ...
        // calculate credit
        IMigrationHelper.CreditDelegationInput[] memory creditDelegations =
            _getCreditDelegations(user, privateKey, positionsToRepay);
    ...
    function _checkMigratedSupplies(address user, address[] memory suppliedPositions, uint256[] memory suppliedBalances)
        internal
    {
        for (uint256 i = 0; i < suppliedPositions.length; i++) {
            ...
            (uint256 currentATokenBalance,,,,,,,,) = v3DataProvider.getUserReserveData(suppliedPositions[i], user);

            assertTrue(currentATokenBalance >= suppliedBalances[i], "supply not migrated");
        }
    }

    function _checkMigratedBorrowings(address user, IMigrationHelper.RepayInput[] memory borrowedPositions) internal {
        for (uint256 i = 0; i < borrowedPositions.length; i++) {
            ...
            (,, uint256 currentVariableDebt,,,,,,) = v3DataProvider.getUserReserveData(borrowedPositions[i].asset, user);
        ...
        returns (address[] memory, uint256[] memory, IMigrationHelper.RepayInput[] memory)
    {
        ...
        IMigrationHelper.RepayInput[] memory borrowedPositions =
            new IMigrationHelper.RepayInput[](v2Reserves.length * 2);
        for (uint256 i = 0; i < v2Reserves.length; i++) {
            ...
            if (currentStableDebt != 0) {
                borrowedPositions[numberOfBorrowed] =
                    IMigrationHelper.RepayInput({asset: v2Reserves[i], amount: currentStableDebt, rateMode: 1});
                numberOfBorrowed++;
            }
            if (currentVariableDebt != 0) {
                borrowedPositions[numberOfBorrowed] =
                    IMigrationHelper.RepayInput({asset: v2Reserves[i], amount: currentVariableDebt, rateMode: 2});
                numberOfBorrowed++;
            }
        }
    ...
    function _getSimplePositionsToRepay(IMigrationHelper.RepayInput[] memory positionsToRepay)
        internal
        pure
        returns (IMigrationHelper.RepaySimpleInput[] memory)
    {
        IMigrationHelper.RepaySimpleInput[] memory positionsToRepaySimple =
            new IMigrationHelper.RepaySimpleInput[](positionsToRepay.length);
    ...
    function _getFlashloanParams(IMigrationHelper.RepayInput[] memory borrowedPositions)
        internal
    ...
    function _getPermits(
        ...
        IMigrationHelper.PermitInput[] memory permits = new IMigrationHelper.PermitInput[](suppliedPositions.length);
    ...
    function _getCreditDelegations(
        ...
        IMigrationHelper.CreditDelegationInput[] memory creditDelegations =
            new IMigrationHelper.CreditDelegationInput[](positionsToRepay.length);

        // calculate params for v3 credit delegation
        (address[] memory borrowedAssets, uint256[] memory borrowedAmounts,) = _getFlashloanParams(positionsToRepay);
        ...
        for (uint256 i = 0; i < borrowedAssets.length; i++) {
            ...
            DataTypes.ReserveData memory reserveData = migrationHelper.V3_POOL().getReserveData(borrowedAssets[i]);
