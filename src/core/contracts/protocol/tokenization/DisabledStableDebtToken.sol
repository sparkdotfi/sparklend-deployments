...
import {IInitializableDebtToken} from "aave-v3-core/contracts/interfaces/IInitializableDebtToken.sol";
...
import {DebtTokenBase} from "aave-v3-core/contracts/protocol/tokenization/base/DebtTokenBase.sol";
...
/**
 * @title DisabledStableDebtToken
 * @author HypurrFi
 * @notice Implements a stable debt token to track the borrowing positions of users
 * at stable rate mode
 * @dev Transfer and approve functionalities are disabled since its a non-transferable token
 */
contract DisabledStableDebtToken is DebtTokenBase, IncentivizedERC20, IStableDebtToken {
    ...
    /**
     * @dev Functions marked by this modifier are disabled.
     */
    modifier disabled() {
        require(false, Errors.STABLE_BORROWING_NOT_ENABLED);
        _;
    }
     ...
     */
    function _mint(address account, uint256 amount, uint256 oldTotalSupply) internal disabled {
     ...
     */
    function _burn(address account, uint256 amount, uint256 oldTotalSupply) internal disabled {
        uint128 castAmount = amount.toUint128();
        uint128 oldAccountBalance = _userState[account].balance;
        _userState[account].balance = oldAccountBalance - castAmount;
     ...
     */
    function transfer(address, uint256) external virtual override disabled returns (bool) {
    ...
    function approve(address, uint256) external virtual override disabled returns (bool) {
        revert(Errors.OPERATION_NOT_SUPPORTED);
    }

    function transferFrom(address, address, uint256) external virtual override disabled returns (bool) {
        revert(Errors.OPERATION_NOT_SUPPORTED);
    }

    function increaseAllowance(address, uint256) external virtual override disabled returns (bool) {
        revert(Errors.OPERATION_NOT_SUPPORTED);
    }

    function decreaseAllowance(address, uint256) external virtual override disabled returns (bool) {
        revert(Errors.OPERATION_NOT_SUPPORTED);
    }
}
