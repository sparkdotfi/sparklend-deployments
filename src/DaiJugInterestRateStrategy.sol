...
contract DaiJugInterestRateStrategy is IReserveInterestRateStrategy {
    ...
    function recompute() public {
        ...
        uint256 debtRatio =
            Art > 0 ? ((_line > 0 && VatLike(vat).live() == 1) ? Art * rate / _line : type(uint88).max) : 0;
    ...
    function calculateInterestRates(DataTypes.CalculateInterestRatesParams memory params)
        ...
        view
        override
        returns (uint256 supplyRate, uint256 stableBorrowRate, uint256 variableBorrowRate)
    {
        ...
        if (debtRatio <= WAD) {
            ...
            if (outstandingBorrow > performanceBonus) {
                ...
                supplyRate = (baseRate + supplySpread) // Flat rate
