...
contract HyperTestnetReservesConfigs {
    ...
    function _faucetTokens(address[] memory tokens, uint256[] memory amounts, address[] memory recipients) internal {
    ...
    function _supplyPool(address[] memory tokens, uint256[] memory amounts, address onBehalfOf) internal {
        for (uint256 i; i < tokens.length;) {
            // approve token amount
            MintableLimitERC20(tokens[i]).approve(address(_getPoolInstance()), amounts[i]);

            // supply tokens
            _getPoolInstance().supply(tokens[i], amounts[i], onBehalfOf, 0);
